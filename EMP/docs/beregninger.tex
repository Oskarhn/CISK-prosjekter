...
\section{Energibalanse og termiske betraktninger}

\subsection{Energibevaring}
Den totale energien $E_0 = 80\,\text{J}$ fordeles under utladningen mellom:
\begin{itemize}
\item Magnetisk feltenergi i spolen (midlertidig lagret, returneres til kretsen)
\item Varmeutvikling i ohmsk motstand (ledninger, spole, ESR)
\item Varme i lysbuen (gnistgap)
\item Utstrålt elektromagnetisk energi (liten andel)
\end{itemize}
En fullstendig energibalanse krever en detaljert modell av lysbuen og nøyaktige målinger av strøm og spenning. Forenklede beregninger som tildeler hele energien til én komponent er ikke fysisk korrekte.

\subsection{Termiske estimater (hypotetiske)}
Følgende beregninger er kun illustrative og forutsetter at en viss andel av energien går til den aktuelle komponenten. De kan ikke brukes til å erklære komponentene som trygge.

\subsubsection{Spole}
Dersom 20 % av energien (16 J) går til oppvarming av spolen (masse ~89 g), blir temperaturøkningen:
\begin{equation}
\Delta T \approx \frac{16}{0.089 \cdot 385} \approx 0.47\,\text{K}.
\end{equation}

\subsubsection{Kondensator}
Dersom 30 % av energien (24 J) tapes i kondensatorens ESR, og kondensatoren veier ~50 g med varmekapasitet ~50 J/K, blir $\Delta T \approx 0.48\,\text{K}$.

\subsection{Konklusjon termisk}
De termiske estimatene er svært usikre og avhenger av energifordelingen som ikke er kjent. Lokale varmepunkter, gjentatte pulser og komponentenes transienttoleranse er ikke vurdert. **Sikker drift kan ikke garanteres uten eksperimentell verifikasjon.**
